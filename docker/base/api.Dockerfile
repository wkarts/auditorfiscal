...
RUN set -eux; \
    ...
    apk add --no-cache ca-certificates curl icu-libs libxml2 libzip oniguruma postgresql-libs rabbitmq-c; \
    apk add --no-cache --virtual .build-deps $PHPIZE_DEPS curl-dev icu-dev libxml2-dev libzip-dev linux-headers oniguruma-dev postgresql-dev rabbitmq-c-dev; \
    docker-php-ext-install -j"$(nproc)" bcmath curl dom intl mbstring opcache pcntl pdo_pgsql sockets xml xmlwriter zip; \
