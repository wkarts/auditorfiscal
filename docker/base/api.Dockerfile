...
RUN set -eux; \
    ...
    docker-php-ext-install -j"$(nproc)" bcmath curl dom gd intl mbstring opcache pcntl pdo_pgsql sockets soap xml xmlwriter zip; \
    pecl install redis amqp; docker-php-ext-enable redis amqp; php -m | grep -Fxq sockets; php -m | grep -Fxq soap; \
