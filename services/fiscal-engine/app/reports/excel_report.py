...
def build_excel(path:Path,company:dict,period:dict,summary:dict,documents:list[dict],findings:list[dict],catalog:dict,parameter_issues:list[dict]|None=None):
    ...
    fmt={'title':wb.add_format({'bold':True,'font_size':18,'font_color':'#FFFFFF','bg_color':'#12324A','align':'left','valign':'vcenter'}),'subtitle':wb.add_format({'font_color':'#FFFFFF','bg_color':'#12324A'}),'header':wb.add_format({'bold':True,'font_color':'#FFFFFF','bg_color':'#137D7B','border':1,'align':'center','valign':'vcenter','text_wrap':True}),'cell':wb.add_format({'border':1,'border_color':'#D8E1E7','valign':'top'}),'text':wb.add_format({'border':1,'border_color':'#D8E1E7','valign':'top','num_format':'@'}),'money':wb.add_format({'border':1,'border_color':'#D8E1E7','num_format':MONEY}),'date':wb.add_format({'border':1,'border_color':'#D8E1E7','num_format':'dd/mm/yyyy'}),'kpi':wb.add_format({'bold':True,'font_size':15,'font_color':'#12324A','bg_color':'#EAF2F7','border':1,'align':'center','valign':'vcenter'}),'label':wb.add_format({'bold':True,'font_color':'#496171','bg_color':'#EAF2F7','border':1,'align':'center'}),'critical':wb.add_format({'bg_color':'#FCE5E3','font_color':'#9B1C13','border':1}),'warning':wb.add_format({'bg_color':'#FFF0CC','font_color':'#7B5700','border':1})}
...
def _write_table(ws,headers,rows,widths,fmt,name):
    ...
    for r,row in enumerate(rows,1):
        for c,v in enumerate(row):
            ...
            if v is not None and _text_identifier(header):ws.write_string(r,c,str(v),fmt['text'])
            else:ws.write(r,c,v,fmt['money'] if isinstance(v,(float,int)) and any(x in header for x in ['valor','base','ibs','cbs','pis','cofins','diferença','custo','venda','margem']) else fmt['cell'])
...
def _reconciliation_sheet(wb,fmt,docs):
    ...
    for row in build_product_reconciliation(docs):
        rows.append([row['identifier'],_identity_type(row['identity_type']),_confidence(row['confidence']),row.get('ncm'),row.get('unit'),float(row['input_quantity']),float(row['output_quantity']),float(row['input_value']),float(row['output_value']),float(row['estimated_cost']) if row['estimated_cost'] is not None else None,float(row['margin']) if row['margin'] is not None else None,_reconciliation_status(row['status'])])
...
def _text_identifier(header):return header in {'nf','ex'} or any(term in header for term in ['chave','emitente','destinatário','ncm','cst','class','cfop','gtin','código','codigo','identificador','cnpj','cpf','sha-256'])
def _identity_type(value):return {'chassis':'Chassi','imei':'IMEI','serial':'Série','aggregation_code':'Agregação','lot':'Lote','gtin':'GTIN','ncm_description':'NCM + descrição'}.get(value,value)
def _confidence(value):return {'exact':'Exata','high':'Alta','indicative':'Indicativa'}.get(value,value)
def _reconciliation_status(value):return {'in_stock':'Em estoque/sem saída','missing_input':'Entrada ausente na amostra','insufficient_quantity_data':'Quantidade insuficiente','insufficient_input_quantity':'Entradas/estoque insuficientes','review_identity':'Revisar correspondência','negative_margin':'Margem negativa','zero_margin':'Margem zero','reconciled':'Conciliado','reconciled_estimate':'Estimativa conciliada'}.get(value,value)
