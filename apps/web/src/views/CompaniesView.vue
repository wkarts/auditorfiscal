...
async function load(){
  const clientsRequest=api.get('/clients',{params:{per_page:100}});
  const accountsRequest=auth.isPlatformAdmin?api.get('/accounts',{params:{per_page:100}}):Promise.resolve(null);
  const [clientsResponse,accountsResponse]=await Promise.all([clientsRequest,accountsRequest]);
  rows.value=clientsResponse.data.data;
  accounts.value=accountsResponse?.data.data||[];
  if(!form.value.account_id)form.value.account_id=auth.user?.tenant_id||accounts.value[0]?.id||'';
}
onMounted(load);
...
<template><div class="split">
  <section class="card">
    ...
    <div class="table-scroll"><table><thead><tr><th>Cliente</th><th v-if="auth.isPlatformAdmin">Empresa da plataforma</th><th>CNPJ</th><th>IE</th><th>Status</th><th></th></tr></thead><tbody>
      <tr v-for="client in rows" :key="client.id"><td>{{client.legal_name}}<small v-if="client.trade_name"><br>{{client.trade_name}}</small></td><td v-if="auth.isPlatformAdmin">{{client.tenant?.trade_name||client.tenant?.legal_name}}</td><td><code>{{client.tax_id}}</code></td><td>{{client.state_registration||'—'}}</td><td>{{client.active?'Ativo':'Inativo'}}</td><td><button v-if="canManage" @click="edit(client)">Editar</button></td></tr>
    </tbody></table></div>
  </section>
  <form v-if="canManage" class="card form-grid one" @submit.prevent="save">
