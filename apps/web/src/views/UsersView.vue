...
<template><div class="split">
  <section class="card">
    ...
    <div class="table-scroll"><table><thead><tr><th>Nome</th><th>E-mail</th><th v-if="auth.isPlatformAdmin">Empresa</th><th>Perfil</th><th>Clientes permitidos</th><th>Auditorias visíveis</th><th>Presença</th><th>Status</th><th></th></tr></thead><tbody>
      <tr v-if="loading"><td :colspan="auth.isPlatformAdmin?9:8">Carregando usuários...</td></tr>
      <tr v-else-if="!users.length"><td :colspan="auth.isPlatformAdmin?9:8">Nenhum usuário encontrado.</td></tr>
      <template v-else><tr v-for="user in users" :key="user.id"><td>{{user.name}}</td><td>{{user.email}}</td><td v-if="auth.isPlatformAdmin">{{user.account?.trade_name||user.account?.legal_name||'Master da plataforma'}}</td><td>{{user.roles.map((role:any)=>role.name).join(', ')}}</td><td>{{user.tenant_id?(hasAllClients(user)?'Todos os clientes':`${(user.clients||[]).length} cliente(s)`):'Todas as empresas e clientes'}}</td><td>{{user.analysis_visibility==='all'||user.roles?.some((role:any)=>role.name==='Administrador')?'Todas as auditorias':'Somente as próprias'}}</td><td><span :class="['badge',user.online?'badge-completed':'badge-draft']">{{user.online?'Online':'Offline'}}</span><small v-if="user.last_seen_at"><br>{{new Date(user.last_seen_at).toLocaleString('pt-BR')}}</small></td><td>{{user.active?'Ativo':'Inativo'}}</td><td><button v-if="user.tenant_id" @click="edit(user)">Editar</button><span v-else class="hint">Conta master</span></td></tr></template>
    </tbody></table></div>
