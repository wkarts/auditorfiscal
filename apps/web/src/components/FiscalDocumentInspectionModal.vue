...
function entries(value:any){return Object.entries(value||{}).filter(([,value])=>value!==''&&value!==null&&value!==undefined&&!(typeof value==='object'&&!Array.isArray(value)&&Object.keys(value).length===0))}
    ...
    <section class="modal fiscal-inspection-modal" role="dialog" aria-modal="true" aria-label="Detalhamento completo do documento fiscal">
      ...
      <template v-else-if="document">
        ...
        <main class="inspection-body">
          ...
          <section v-show="tab==='items'" class="inspection-section"><h3>Dados dos produtos e serviços</h3><div class="table-scroll"><table><thead><tr><th>Item</th><th>Produto / serviço</th><th>NCM</th><th>CFOP</th><th>Quantidade</th><th>Unidade</th><th>Valor unitário</th><th>Valor total</th></tr></thead><tbody><template v-for="item in document.items" :key="item.id"><tr><td>{{item.item_number}}</td><td><strong>{{item.description||'—'}}</strong><small v-if="item.product_code"><br>Código: {{item.product_code}}</small></td><td>{{item.ncm||'—'}}<template v-if="item.ex_code"> / {{item.ex_code}}</template></td><td>{{item.cfop||'—'}}</td><td>{{item.details?.quantity||'—'}}</td><td>{{item.details?.unit||'—'}}</td><td>{{item.details?.unit_value||'—'}}</td><td>{{money(item.product_value)}}</td></tr><tr class="inspection-item-details"><td></td><td colspan="7"><details><summary>Tributos, rastreabilidade, identificadores e informações adicionais</summary><div class="item-detail-grid"><dl class="inspection-fields compact"><template v-for="[key,value] in entries(item.details||{})" :key="key"><dt>{{label(key)}}</dt><dd><pre v-if="typeof value==='object'">{{valueOf(value)}}</pre><template v-else>{{valueOf(value)}}</template></dd></template></dl><dl class="inspection-fields compact"><template v-for="[key,value] in entries(item.tax_components||{})" :key="key"><dt>{{label(key)}}</dt><dd>{{money(value)}}</dd></template></dl></div></details></td></tr></template><tr v-if="!document.items.length"><td colspan="8" class="empty-state">Este modelo não possui itens de produto normalizados.</td></tr></tbody></table></div></section>
